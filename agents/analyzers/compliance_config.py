...
class ComplianceConfigManager:
    ...
    def _load_compliance_templates(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            'hs_household_shelter': {
                ...
                'key_columns': [
                    'No', 'Clause', 'Parameter', 'Min_Rectilinear_Area', 
                    'Min_Irregular_Area', 'Unit', 'Min_Volume', 'Unit_Area', 
                    'HS_Area', 'HS_Volume', 'HS_Slab_Thickness', 
                    'HS_Staircase_Thickness', 'Compliance_Status', 
                ...
                'domain_patterns': {
                    ...
                    'HS_Slab_Thickness': ['hs slab thickness', 'ceiling thickness', 
                                          'slab thickness'],
                    'HS_Staircase_Thickness': ['waist thickness', 'staircase thickness',
                                               'stair waist']
                }
            },
            'fire_safety': {
    ...
    def _load_hs_scenarios(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if scenarios_path.exists():
                with open(scenarios_path, 'r', encoding='utf-8') as file_handle:
                    scenarios_data = yaml.safe_load(file_handle)
            ...
            print(f"[ERROR] Failed to load HS scenarios: {exception}")
            return {'scenarios': {}, 'default_scenario': 'residential_standard'}
    ...
    def set_compliance_domain(self, domain: str) -> bool:
        ...
        if domain in self.compliance_templates:
            ...
            template_name = self.compliance_templates[domain]['name']
            print(f"[DEBUG] Set compliance domain to: {template_name}")
            return True
        else:
