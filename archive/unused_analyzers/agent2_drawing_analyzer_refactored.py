...
class DrawingAnalysisAgent:
    ...
    def _perform_ai_analysis(self, parameters_df, valid_images: List[str],
        ...
        # Save debug info
        self.api_client.save_debug_info(
            self.prompt["system"], user_prompt, len(valid_images), len(dxf_files)
        )
    ...
    def _parse_and_process_response(self, content: str, parameters_df,
        ...
        if success:
            ...
            return self._fallback_json_parsing(content, parameters_df, valid_images, dxf_files)
    
    def _fallback_json_parsing(self, content: str, parameters_df,
                             valid_images: List[str], dxf_files: List[str]) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            result_info = f"AI analyzed {len(valid_images)} image files"
            if dxf_files:
            ...
            return False, {"error": f"Failed to parse AI response: {str(exception)}"}
